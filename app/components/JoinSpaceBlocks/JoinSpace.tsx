...
import { JoinSpaceProps } from '../../types/join-space';
import { joinSpace } from '../../services/iskolarspace-api';
import { useRouter } from 'next/navigation';

const JoinSpace = ({ userId }: JoinSpaceProps) => {
  ...
  const handleJoin = async () => {
    ...
    setSuccess("");
    try {
  const apiResult = await joinSpace(code, userId);
  setSuccess("Successfully joined the space!");
  setCode("");
  const spaceId = apiResult?.spaceId || code;
  ...
  return (
    <div className='flex mt-20 bg-black/30 rounded-xl outline-none focus:outline-none focus:ring-0 p-2 sm:p-2 md:p-7 lg:p-8 flex-col items-center justify-center mx-auto h-full'>
      ...
      <div className="flex items-center bg-[#1c2a4d] rounded-full px-4 sm:px-4 md:px-6 sm:py-4 md:py-5 min-w-0 w-full mb-2">
        ...
        />
        <button
          className="ml-4 bg-blue-400 hover:bg-blue-500 text-white font-semibold px-6 sm:px-6 py-2 md:py-2 rounded-full"
          onClick={handleJoin}
